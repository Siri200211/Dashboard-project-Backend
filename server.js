...
app.post('/upload-csv', upload.single('sales_details_2024'), async (req, res) => {
    ...
    .on('end', async () => {
      try {
        if (replaceData) {
          ...
          const lastSavedDate = await Order.findOne().sort({ date: -1 }).select('date'); // Get the most recent date
          ...
          const newData = results.filter(item => moment(item.date).isAfter(lastSaved));
...
app.post('/upload-disconnection-csv', upload.single('disconnection_file'), async (req, res) => {
  const replaceData = req.query.replaceData === "true"; // Check if full replace is requested
  ...
  try {
    ...
    readableStream
      ...
      .on('data', (data) => {
        try {
          // Parse the dates with strict validation
          const churnDate = moment(data['Churn_Date'], "MMMM D, YYYY", true).toDate();
          const activatedDate = moment(data['Activated_Date'], "MMMM D, YYYY", true).toDate();

          // Skip rows with invalid dates
          if (!churnDate || isNaN(churnDate)) {
            console.warn(`Invalid Churn_Date: '${data['Churn_Date']}' - Skipping row.`);
            return;
          }
          if (!activatedDate || isNaN(activatedDate)) {
            console.warn(`Invalid Activated_Date: '${data['Activated_Date']}' - Skipping row.`);
            return;
          }
          ...
          console.error("Error parsing row:", parseError.message, "Row:", data);
        }
      })
      .on('end', async () => {
        try {
          ...
          if (replaceData) {
            ...
            const lastSaved = await Disconnection.findOne().sort({ churn_date: -1 }).select('churn_date');
            const lastSavedDate = lastSaved ? lastSaved.churn_date : new Date(0);

            console.log("Last saved churn date:", lastSavedDate);

            const newData = results.filter((item) => moment(item.churn_date).isAfter(lastSavedDate));
            if (newData.length > 0) {
              console.log(`Inserting ${newData.length} new records...`);
              await Disconnection.insertMany(newData);
              return res.send('New disconnection records inserted successfully.');
            } else {
...
app.get('/get-peo-tv-counts', async (req, res) => {
  ...
  if (year) matchStage.$and.push({ $expr: { $eq: [{ $year: "$churn_date" }, Number(year)] } });
  if (month) matchStage.$and.push({ $expr: { $eq: [{ $month: "$churn_date" }, Number(month)] } });
  if (day) matchStage.$and.push({ $expr: { $eq: [{ $dayOfMonth: "$churn_date" }, Number(day)] } });
  ...
  if (deleted_method) {
    if (deleted_method === "Customer Requested") {
      matchStage.$and.push({
        deleted_method: { $in: ["Customer Requested", "Promotion Downgrade", "Promotion Upgrade"] },
      });
        ...
        $project: {
          ...
          peo_tv_disconnections: {
            $cond: [
              {
                $and: [
                  { $setEquals: [{ $setDifference: ["$oss_types", ["E-IPTV FTTH", "E-IPTV COPPER"]] }, []] },
                ],
              },
              "$total_occurrences",
              0,
